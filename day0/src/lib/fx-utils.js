...
// Assume utilsRand by default but fallback to Math.random() as our PRNG
var utilsRand;
refreshRandomFn();

export function refreshRandomFn() {
  utilsRand = window.fxrand ?? Math.random;
}
...
export function pickNofM(n, arr) {
  ...
  for (let i = 0; i < n; i++) {
    let c = cp.splice(Math.floor(utilsRand() * cp.length), 1);
...
export function pick(arr) {
  return arr[Math.floor(utilsRand() * arr.length)];
}
